...
# This script sends event data with a 'status' that can be either "normal" or "anomaly".
...
# Get the producer ID from environment variable or default to "3"
producer_id = os.getenv("PRODUCER_ID", "3")
topic = "anomaly-events"

# Configure the Kafka producer. The bootstrap.servers should point to your Kafka broker.
conf = {
...
try:
    print(f"Starting producer {producer_id}... sending events to topic '{topic}'")
    while True:
        # Create a dictionary for the new event structure
        event = {
            "producer_id": producer_id,
            "event_id": randint(1000, 9999),
            "anomaly_score": round(randint(0, 100)/100, 2),
            "status": choice(["normal", "anomaly"]),
            "source_ip": f"192.168.{randint(0,255)}.{randint(0,255)}",
            "timestamp": time.time()
        }
